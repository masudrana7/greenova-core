...
?>
<div class="rt-info-text-4">
	<div class="service-box">
		<?php if ( $icontype == 'image' ) { ?>
				<?php echo wp_get_attachment_image( $image, array( $size, $size ), true ); ?>
			<?php } else { ?>

			<span><?php if ( !empty( $url ) ) { ?><a href="<?php echo esc_url( $url ); ?>"><?php } ?><i style="color:<?php echo esc_attr($color); ?>;<?php echo esc_attr($icon_css); ?>" class="<?php echo esc_attr( $icon ); ?>" aria-hidden="true"></i><?php if ( !empty( $url ) ) { ?></a><?php } ?>            
			<?php } ?></span>

		<h3><?php if (!empty($url)) { ?><a href="<?php echo esc_attr($url); ?>"><?php } ?><?php echo wp_kses_post( $heading ); ?><?php if (!empty($url)) { ?></a><?php } ?></h3>
		<p style="<?php echo esc_attr( $content_css );?>"><?php echo wp_kses_post( $content ); ?></p>
		<?php if ( !empty( $button_url ) ) { ?>
		<a class="btn-quote2" href="<?php echo esc_url( $button_url ); ?>"><?php echo wp_kses_post( $button_text ); ?></a>
		<?php } ?>
	</div>
</div>
