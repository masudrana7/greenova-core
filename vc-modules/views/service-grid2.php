...
$wp_query   = $query;
?>
<div class="row auto-clear rt-service-grid-2">
<?php if ( have_posts() ) { ?>
	...
	<div class="service-box <?php echo esc_attr($col_class); ?>">
		<div class="service-img-holder">
			...
			<div class="service-hover-content-holder">
				<?php if ( $showlink == 'true' ) { ?>
				<h3 class="title-bar35"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
				<?php } else { ?>
	...
	<?php endwhile;?>	
		...
		<?php if ( $button_display == 'true' ) { ?>
			<div class="rt-grid-fill-btn">
				<a href="<?php echo esc_url( $buttonurl );?>" class="grid-fill-btn"><?php echo esc_html( $buttontext );?></a>
			</div>
		<?php } ?>
	<?php } else { ?>
