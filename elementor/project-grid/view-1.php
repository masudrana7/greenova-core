...
?>
<div class="rt-project-gallery-9 rt-project-grid-wrapper">
    <div class="row auto-clear entry-content">
		<?php if ( have_posts() ) { ?>
			<?php while ( have_posts() ) : the_post();
                ...
                <div class="project9-box <?php echo esc_attr( $col_class ); ?>">
                    <div class="rt-portfolio-item project-inner-wrapper">
						<?php if ( $showlink ) { ?>
							<?php echo wp_kses_post( $thumbnail ); ?>
						<?php } else { ?>
							<?php echo wp_kses_post( $thumbnail ); ?>
						<?php } ?>
                        <div class="rt-content project-content-wrapper">
                            <div class="rt-content-holder">
								<?php if ( $showlink ) { ?>
								...
								<?php if ( 'visible' == $data['readmore_visibility'] ) : ?>
                                    <a class="grid9-btn read-more-btn" href="<?php the_permalink(); ?>"><?php echo esc_html( $data['readmore_text'] ); ?></a>
								<?php endif; ?>
                            </div>
                        </div>
                    </div>
